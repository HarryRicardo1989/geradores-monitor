...
class DATABASE:
    ...
    def insert_DB(self, hostname, data_hora, temperatura_ar, temperatura_orvalho, umidade, pressao_local, correnteFaseA,correnteFaseB, correnteFasec, correnteNeutro, statusEnergia):
        ...
        cur = conn.cursor()
        try:
            cur.execute("INSERT INTO gerador_dados_tbl ( hostname, data_hora, temperatura_ar, temperatura_orvalho, umidade, pressao_local,correnteFaseA,correnteFaseB, correnteFasec, correnteNeutro,statusEnergia) VALUES (?, ?, ?, ?, ?, ?,?,?,?,?,?)",
    ...
    def select_DB(self, horas_de_coleta=1.0):
        ...
        cur = conn.cursor()
        cur.execute(
            f'SELECT hostname, data_hora, temperatura_ar, temperatura_orvalho, umidade, pressao_local FROM gerador_dados_tbl where data_hora > "{tempo}" order by data_hora desc;')
